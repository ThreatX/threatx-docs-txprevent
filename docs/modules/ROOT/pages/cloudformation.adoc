...
=== 🎛️ Threa️tX Control Plane Services
...
* [ ] 🗝️ *Valid ThreatX API Key*  (See http://docs.threatx.com/txgeneral/admin_guide.html#generating-and-revoking-sensor-api-keys[ThreatX Sensor API Key])
...
include::partial$cloudformation-template-params.adoc[]
...
Follow these steps ️to deploy the CloudFormation stack to create the {product-name} services in your AWS environment.
...
==== 2️⃣ Configure the Stack Details

__📍 On the 📄 *Specify stack details* Page__
...
[,console]
----
include::example$launch-docker-container.sh[]
----
...
.ℹ️ Common Network Interface Names for Linux Distributions
****
